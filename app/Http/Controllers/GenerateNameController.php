...
class GenerateNameController extends Controller
{
    ...
    public function store(Request $request)
    {
      ...
      foreach ($relevantfirstnames as $entry) {
        ...
        if($entry['rank'] > $bestfirstnamerank) {
          $bestfirstnamerank = $entry['rank'];
          $bestfirstname = $entry['Firstname'];
        }
      ...
      foreach ($relevantlastnames as $entry) {
        ...
        if($entry['rank'] > $bestlastnamerank) {
          $bestlastnamerank = $entry['rank'];
          $bestlastname = $entry['Lastname'];
        }
        // Add to new list of results for sort
      ...
      if (isset($bestfirstname) == false || isset($bestlastname) == false) {
      ...
      $charname = $bestfirstname . ' ' . $bestlastname;
    ...
    public function random(Request $request)
    {
      ...
      $randkeyrace = array_rand($raceoptions, 1);
      $inputrace = $raceoptions[$randkeyrace];
      ...
      $randkeygender = array_rand($genderoptions, 1);
      $inputgender = $genderoptions[$randkeygender];
      ...
      $randkeysfirstname = array_rand($relevantfirstnames, 1);
      $firstname = $relevantfirstnames[$randkeysfirstname]['Firstname'];

      // Select random positions from last name options
      $randkeyslastname = array_rand($relevantlastnames, 1);
      $lastname = $relevantlastnames[$randkeyslastname]['Lastname'];
